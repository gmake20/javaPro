...
public class Ex09_scm {
	...
	public static void main(String[] args) {
		...
		processStudentRank(students, index);
	...
	private static void printStudentInfo(Student[] students, int index) {
		System.out.printf("\n 입력받은 학생수: %d명\n", index);
		for (int i = 0; i < index; i++) {
			System.out.printf(
					"> %d번 이름: %s, 국어:%d, 영어:%d, 수학:%d, 총점:%d, 평균:%.2f, 등수:%d\n"
					, i+1, students[i].name, students[i].kor, students[i].eng, students[i].mat, students[i].tot, students[i].avg, students[i].rank);
		}
	...
	private static void processStudentRank(Student[] students,  int index) {
		// i=0
		// j=0123
		// i=1
		// j=0123
